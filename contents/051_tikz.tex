...
\begin{frame}
  \frametitle{Why creating figures in \LaTeX?}

  \begin{itemize}
    \item[$+$] Adjustability/Flexibility\\
    Rebuilding your document automatically updates the figure: making changes is as easy as modifying \LaTeX\ code
      \pause
    \item[$+$] Consistency:\\
        Fonts, colors, text sizes, etc., are consistent.
      \pause 
    \item[$+$] Vector graphics
      \pause
    \item[$+$] Easy to port your text to a slide deck
      \pause
    \item[$-$] The learning curve\ldots
    \item[$-$] Compiling can become slow
  \end{itemize}
  \let\thefootnote\relax\footnotetext{Inspired by the Tikz workshop by Pieter Belmans (\url{https://github.com/pbelmans/tikz-workshop}).}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
      \item \url{TeXample.net}
      \item \url{PGFPlots.net}
      \item \href{https://pgfplots.sourceforge.net/gallery.html}{PGFPlots Gallery}
       \item \href{https://www.bsaver.io/misc/pretty-figures}{Pretty figures (Tool from ETH Zurich)}
  \end{itemize}
  \begin{figure}
      \centering
      \includegraphics[width=0.4\linewidth]{graphics/exqmple-tikz-tool.png}
  \end{figure}
\end{frame}
...
\begin{frame}
  \frametitle{Integration with External Tools}
  \textbf{I would not recommend the following anymore:}*
  
  \begin{description}
    \item[\texttt{matlab2tikz}] \url{https://github.com/nschloe/matlab2tikz}
      Export Matlab figures directly to \TikZ.
    \item[\texttt{matplotlib2tikz}] \url{https://github.com/nschloe/matplotlib2tikz}
      Export Matplotlib figures directly to \TikZ.
  \end{description}
  *use CSV to plot, making it easier to update later.
\end{frame}

\begin{frame}
  \frametitle{Tikz Externalize (in Overleaf)}

  \begin{itemize} 
      \item \TikZ is recompiled every run
      \item externalize to save as PDF
      \item \commopt{tikzexternalize}{prefix=tikz/} command must be given in the project’s main \texttt{.tex} file
      \item Ensure in Overleaf that \texttt{tikz/} is created and contains a dummy file
      \item Set a filename for each \TikZ picture\\
      \commopt{tikzsetnextfilename}{file name}
      \item More information on the Overleaf Documentation.
  \end{itemize}
\end{frame}
